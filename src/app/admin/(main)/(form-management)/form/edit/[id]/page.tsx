...
import { fetchAllTypeList, fetchDetailForm, fetchEditForm } from "@/store/thunks/admin/formThunk";
...
const EditForm = ({ params }: { params: PageParams}) => {
  const router = useRouter();
  const dispatch = useDispatch<AppDispatch>();
  const {loading, formErrors, success, formDetail} = useSelector((state: RootState) => state.form);
  ...
  useEffect(() => {
    dispatch(fetchDetailForm(id));
    dispatch(fetchAllTypeList());
  }, []);
  
  useEffect(() => {
    if(Object.keys(formDetail).length != 0) {
      setUserOptionId(formDetail.userOption.id.toString());
      dispatch(setSelectedGroups(formDetail.groups));
      dispatch(setSelectedUsers(formDetail.departments));
      imageFormat(formDetail.file);
      formik.setValues({
        name: formDetail.name,
        form_type_id: formDetail.formType.id.toString(),
      })
    }
  }, [formDetail])

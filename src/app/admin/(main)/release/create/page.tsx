...
import { fetchCreateRelease } from "@/store/thunks/admin/releaseThunk";
import ConfirmGroup from "@/components/admin/release/ConfirmGroup";
import { resetImages } from "@/store/slices/admin/mediaSlice";

const CreateRelease = () => {
  const router = useRouter();
  const dispatch = useDispatch<AppDispatch>();
  const {loading, success, releaseErrors} = useSelector((state: RootState) => state.release);
  const {selectedDepartmentUsers} = useSelector((state: RootState) => state.group);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      images.map(file => {
        uploaded_images.push(file.serverId);
      })
      ...
      dispatch(fetchCreateRelease(formData));
    },
    ...
    <div style={{ width: "100%" }}>
      <form onSubmit={formik.handleSubmit}>
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 8 }}>
          <Typography variant="h6">Create Release</Typography>
        </Box>
        <Box sx={{ mb: 4 }}>

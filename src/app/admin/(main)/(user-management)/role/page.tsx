...
import { fetchDeleteRole, fetchRoleList } from '@/store/thunks/admin/roleThunk';
import { resetMessage, updateParams } from '@/store/slices/admin/RoleSlice';
...
const ManageRole = () => {
  ...
  const {loading, roleParams, roleList, success, roleMessage, roleErrors} = useSelector((state: RootState) => state.role);
  const [page, setPage] = useState<number>(roleParams.page);
  const [perPage, setPerPage] = useState<number>(roleParams.perPage);
  const [keyword, setKeyword] = useState<string>(roleParams.keyword);
  const [fromDate, setFromDate] = useState<Dayjs | null>(roleParams.fromDate);
  const [toDate, setToDate] = useState<Dayjs | null>(roleParams.toDate);
  ...
  useEffect(() => {
    dispatch(fetchRoleList(roleParams));
  }, [roleParams]);
  ...
  const handleConfirm = () => {
    dispatch(fetchDeleteRole(currentId));
    handleCloseDialog();
  }
        ...
        if(params.row.name != 'Super Admin')
          return (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Can I={Permission.ROLE_EDIT} a="admin">
                <Button onClick={() => editRole(params.id as number)} size="small" color='secondary' startIcon={<EditOutlinedIcon />} disableRipple>
                  Edit
                </Button>
              </Can>
              <Can I={Permission.ROLE_EDIT} a="admin">
                <Button onClick={() => deleteRole(params.id as number)} size="small" color='secondary' startIcon={<DeleteOutlineOutlinedIcon />} disableRipple>
                  Delete
                </Button>
              </Can>
            </Box>
  ...
  return (
    <div style={{ width: '100%' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        ...
        <Can I={Permission.ROLE_CREATE} a="admin">
          <Button onClick={createRole} variant="contained" color='info' sx={{textTransform: 'none'}}>Add New</Button>
        </Can>
      </Box>
      ...
      <TablePagination
        ...
        component="div"
        count={(roleList.data && roleList.meta.total) || 0} // total Element
        rowsPerPage={perPage}
        page={page}

...
import { fetchDeleteGroup, fetchGroupList } from '@/store/thunks/admin/groupThunk';
import { resetGroupParams, resetMessage, updateParams } from '@/store/slices/admin/groupSlice';
...
const ManageGroup = () => {
  ...
  const {loading, groupParams, groupList, success, groupMessage} = useSelector((state: RootState) => state.group);
  const [page, setPage] = useState<number>(groupParams.page);
  const [perPage, setPerPage] = useState<number>(groupParams.perPage);
  const [keyword, setKeyword] = useState<string>(groupParams.keyword);
  const [fromDate, setFromDate] = useState<Dayjs | null>(groupParams.fromDate);
  const [toDate, setToDate] = useState<Dayjs | null>(groupParams.toDate);
  ...
  useEffect(() => {
    dispatch(fetchGroupList(groupParams));
  }, [groupParams])
  ...
  const handleConfirm = () => {
    dispatch(fetchDeleteGroup(currentId));
    handleCloseDialog();
  }

  const columns: GridColDef[] = [
    { field: 'id', headerName: 'ID', minWidth: 100 },
    { field: 'name', headerName: 'Group Name', minWidth: 200 },
      ...
      renderCell: (params) => {
        return (
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Can I={Permission.GROUP_EDIT} a="admin">
              <Button onClick={() => editGroup(params.id as number)} size="small" color="secondary" startIcon={<EditOutlinedIcon />} disableRipple>
                Edit
              </Button>
            </Can>
            <Can I={Permission.GROUP_DELETE} a="admin">
              <Button onClick={() => deleteGroup(params.id as number)} size="small" color="secondary" startIcon={<DeleteOutlineOutlinedIcon />} disableRipple>
                Delete
              </Button>
            </Can>
          </Box>
    ...
    <div style={{ width: '100%' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        <Typography variant="h6"> Group List </Typography>
          <Can I={Permission.GROUP_CREATE} a="admin">
            <Button onClick={createGroup} variant="contained" color="info" sx={{ textTransform: "none" }}>
              Add New
            </Button>
          </Can>
      </Box>
      ...
      <TablePagination
        ...
        component="div"
        count={(groupList.data && groupList.meta.total) || 0} // total Element
        rowsPerPage={perPage}
        page={page}

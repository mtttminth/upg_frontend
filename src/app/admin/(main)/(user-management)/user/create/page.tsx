"use client";
import { Box, FormControl, FormHelperText, Grid, MenuItem, Select, SelectChangeEvent, TextField, Typography, } from "@mui/material";
import { useFormik } from "formik";
import React, { useEffect } from "react";
import { CreateUserValidationSchema } from "@/validations/schema";
import { useRouter } from "next/navigation";
import { AppDispatch, RootState } from "@/store/store";
import { useDispatch, useSelector } from "react-redux";
import { LoadingButton } from "@mui/lab";
import { Button } from "@mui/material";
import { fetchCreateUser } from "@/store/thunks/admin/userThunk";

const CreateUser = () => {
  const dispatch = useDispatch<AppDispatch>();
  const router = useRouter();
  const {loading, success, userErrors} = useSelector((state: RootState) => state.user);
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} md={4}>
            <FormControl variant="standard" fullWidth>
              <p className="mb-1">
                Staff Code <span className="text-red-600">*</span>
              </p>
              <TextField
                name="code"
                size="small"

...
import { fetchContractList, fetchDeleteContract } from '@/store/thunks/admin/contractThunk';
import { resetMessage, updateContractParams } from '@/store/slices/admin/contract';
import { resetGroupParams } from '@/store/slices/admin/groupSlice';

const Contract = () => {
  const dispatch = useDispatch<AppDispatch>();
  const {loading, contractParams, contractList, success, contractMessage} = useSelector((state: RootState) => state.contract);
  ...
  const [fromDate, setFromDate] = useState<Dayjs | null>(contractParams.fromDate);
  const [toDate, setToDate] = useState<Dayjs | null>(contractParams.toDate);
  ...
  const search = () => {
    dispatch(updateContractParams({page, perPage, keyword, fromDate, toDate}));
  }
  ...
  const createContract = () => {
    dispatch(resetMessage());
    router.push('/admin/contract/create');
  }

  const editContract = (id: number) => {
    dispatch(resetMessage());
    router.push(`/admin/contract/edit/${id}`);
  }
  ...
  const handleConfirm = () => {
    dispatch(fetchDeleteContract(currentId));
    handleCloseDialog();
  }

  const columns: GridColDef[] = [
    { field: 'id', headerName: 'ID', minWidth: 100 },
    { field: 'name', headerName: 'Contract Name', minWidth: 180 },
    { field: 'contractType', headerName: 'Type Name', minWidth: 180 },
      ...
      renderCell: (params) => {
        return (
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Can I={Permission.CONTRACT_EDIT} a="admin">
              <Button onClick={() => editContract(params.id as number)} size="small" color="secondary" startIcon={<EditOutlinedIcon />} disableRipple>
                Edit
              </Button>
            </Can>
            <Can I={Permission.CONTRACT_DELETE} a="admin">
              <Button onClick={() => deleteContract(params.id as number)} size="small" color="secondary" startIcon={<DeleteOutlineOutlinedIcon />} disableRipple>
                Delete
              </Button>
            </Can>
          </Box>
    ...
    <div style={{ width: '100%' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        ...
        <Can I={Permission.CONTRACT_CREATE} a="admin">
          <Button onClick={createContract} variant="contained" color="info" sx={{ textTranscontract: "none" }}>
            Add New
          </Button>
        </Can>
      </Box>
      ...
      <TablePagination
        ...
        component="div"
        count={(contractList.data && contractList.meta.total) || 0} // total Element
        rowsPerPage={perPage}
        page={page}

...
import { CreateSubCategoryValidationSchema, EditAdminValidationSchema } from "@/validations/schema";
import { useRouter } from "next/navigation";
import { AppDispatch, RootState } from "@/store/store";
import { useDispatch, useSelector } from "react-redux";
import { LoadingButton } from "@mui/lab";
import { fetchDetailAdmin, fetchEditAdmin } from "@/store/thunks/admin/adminThunk";
import { fetchAllCategoryList, fetchDetailSubCategory, fetchEditSubCategory } from "@/store/thunks/admin/resourcesThunk";

type PageParams = {
  id: string;
}

const EditSubCategory = ({ params }: { params: PageParams}) => {
  const router = useRouter();
  const dispatch = useDispatch<AppDispatch>();
  const {loading, success, subCategoryDetail, allCategoryList, pageErrors} = useSelector((state: RootState) => state.page);
  ...
  useEffect(() => {
    dispatch(fetchDetailSubCategory(id));
    dispatch(fetchAllCategoryList());
  }, []);

  useEffect(() => {
    if(Object.keys(subCategoryDetail).length != 0) {
      formik.setValues({
        name: subCategoryDetail.name,
        category_id: subCategoryDetail.category?.id+'',
      })
    }
  }, [subCategoryDetail])

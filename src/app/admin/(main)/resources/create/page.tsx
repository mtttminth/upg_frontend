...
import { fetchAllCategoryList, fetchCreatePage, fetchSubCategoryListByCategoryId } from "@/store/thunks/admin/resourcesThunk";

const CreatePage = () => {
  const router = useRouter();
  const dispatch = useDispatch<AppDispatch>();
  const {selectedDepartmentUsers, selectedGroupsDraft} = useSelector((state: RootState) => state.group);
  const {loading, pageErrors, success} = useSelector((state: RootState) => state.page);

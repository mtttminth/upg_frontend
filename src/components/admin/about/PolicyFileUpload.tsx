...
import { resetPolicyFile, updatePolicyFile } from '@/store/slices/admin/mediaSlice';

registerPlugin(FilePondPluginImageExifOrientation, FilePondPluginImagePreview);

const PolicyFileUpload = () => {
  const dispatch = useDispatch<AppDispatch>();
  const {policyFile} = useSelector((state: RootState) => state.media);
  const pond = useRef<FilePond | null>(null);
  const accessToken = tokenService.getAccessToken(ADMIN);

  const handleUpdateFiles = (fileItems: any[]) => {
    dispatch(updatePolicyFile(fileItems));
  };

  useEffect(() => {
    return () => {
      dispatch(resetPolicyFile())
    };
  }, []);
